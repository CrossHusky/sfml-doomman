...
namespace basegame{
	namespace state{
		...
		bool GameState::loadMap()
		{
			...
			{
				mPack->Manager->pushTop(new ErrState("No se pudo cargar el mapa :("));
				return false;
			}
			return true;
		}
		...
		{
			if(!mPack)//Inicializadores de primera instancia
			{
				mTheme.openFromFile("theme.ogg");
				mTheme.play();
				mPack=&pack;
				mapOk=loadMap();
				lives.setFont(*pack.Font);
				score.setFont(*pack.Font);
				lives.setString("Vida(s): 3");
				score.setString("Puntuación: 0");
				lives.setCharacterSize(32);
				score.setCharacterSize(32);
				lives.setPosition(0.f,500.f);
				score.setPosition(0.f,533.f);
			}
			...
			while(mapOk)
			{
				...
				while(mEngine.getEvent(peve))
				{
					...
					case pac::EventType::PacDied:
						lives.setString("Vida(s): "+sup::toString(peve.data));
						break;
					...
					case pac::EventType::ScoreChange:
						mScore+=peve.data;
						score.setString("Puntuacion: "+sup::toString(mScore));
						break;
					}
				}//peve
				pack.Window->clear();
		...
		void GameState::drawGhosts()
		{
			if(mAnimClock.getElapsedTime()>sf::seconds(0.2f))
			{
				...
				//scared/nonscared cambios de color
		...
		void GameState::drawAll()
		{
			...
			//Pacman falsos para que atraviesen el tunel suavemente
