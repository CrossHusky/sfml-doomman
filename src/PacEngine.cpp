...
namespace {
	...
	{
		if (number==basegame::pac::PacEngine::Wall)
			return false;
		return true;
	}
	void drawHorizontalLine(sf::RenderTarget& rt,int x1,int x2,int y) // Para el mapa
	{
		sf::Vertex arr[4];
	...
	void drawVerticalLine(sf::RenderTarget& rt,int y1,int y2,int x) // Para el mapa
	{
		sf::Vertex arr[4];
		arr[0].color=sf::Color::Blue;
		arr[1].color=sf::Color::Blue;
	...
	basegame::pac::PacEntity::eDirection getDirFromVec(sf::Vector2i from)
	{
		if(from.x==1)return basegame::pac::PacEntity::Right;
		if(from.x==-1)return basegame::pac::PacEntity::Left;
		if(from.y==1)return basegame::pac::PacEntity::Down;
		if(from.y==-1)return basegame::pac::PacEntity::Up;
		return basegame::pac::PacEntity::Right;
	}
	bool ClydeChase(basegame::pac::PacEntity pac,basegame::pac::PacEntity cly)
	{
...
namespace basegame{
	namespace pac{
		...
		{
			guys[0].position.x+=16*28;//Para evitar problemas con negativos
			guys[0].position.x%=16*28;//Para el tunel
			...
			if(guys[Pac].isAtNode())
			{
				...
				case Booster:
					...
					mGhostKillStreak=0;// Reinicia el "KillStreak"
					for(int i=Blinky;i<=Clyde;++i)
					{
						if(getScareStatus(i)!=Dead)guys[i].scared=7*60; // 7 Segundos
					}
					break;
				}
		...
		sf::Vector2i PacEngine::getTarg(int who)
		{
			...
			return sf::Vector2i();//default para VC++
		}
